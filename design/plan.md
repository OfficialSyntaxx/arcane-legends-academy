...
**Delivery context:** desktop web + mobile web (both). Keyboard/mouse (`event.code`), touch (joystick + drag-to-rotate camera + pinch-zoom), gamepad. English (all strings external). **Strictness: S.** `draw_call_budget` 80 (mobile), `entity_count_estimate` ~60, `worst_case_scene` = "duel battlefield, full board + hand + overlays".
...
- Card value = rarity base × grade multiplier. Equipment value scales with metal tier.
- Inflation guarded by sinks; grading EV ≈ 0 (genuine gamble).

## Laws / Concept / System / Prototype
- **L1 formula:** The player feels like a rising master wizard building a legendary collection and a thriving magic guild, because the game constantly rewards them with new cards, gold, gear, and dueling decisions that visibly matter.
- **L2:** every action (play, attack, mine, craft, equip, buy, sell, auction, grade, regrade, upgrade) has a visible effect + resurfacing consequence.
- **L3 patterns:** pip economy, summon timing, school synergy, elemental counters, grading speculation, deck building, skill→craft→gear→duel pipeline, market timing.
- **L4 uncertainty:** randomness (draws, packs, grading, regrade, auction floors), hidden info (opponent hand/deck, face-down traps, ungraded rolls), analytic complexity (combat/deck/market).
- **Verbs:** play, attack, end turn, mine, fish, chop, smelt, forge, brew, refine, scribe, equip, buy, sell, auction, grade, regrade, open pack, build deck, quest, upgrade home, matchmake.
- **Information map:** own hand/cards/inventory/equipment open; opponent hand/deck/traps hidden (masked in `viewFor`); HP open; card rolls hidden until graded; market prices open.

## Balance (numbers in data, reconciled in code)
- Creature power ≈ cost+1; spell dmg ≈ cost×2. Equipment scales with metal tier.
- Cost curve: higher-tier gear & cards cost more per unit of power.
- Non-transitive elemental ring in schools; skill/progression curves gentle-decelerate.
- Economy: sinks keep pace with sources; remainder non-monotonic.