...
1. **Generated character models are integrated.** The player wizard (2D→3D), professor (static), and merchant (skinned) all load at ~1.8 units and render correctly. The fix in `makeCharModel` (`world.js`): for **skinned Meshy GLBs** the object box is degenerate (0) and the raw mesh box is only the *bind pose* — the real size is the **skeleton node span** (bones sit far above the mesh), so height is computed from node world positions. For **static meshes** (e.g. professor.glb has no skeleton), the real size is the **geometry box**, so it's used instead. The loader auto-detects skinned vs static. The remaining NPCs (referee, trainer, librarian, wanderers) are still procedural and can be generated via 2D→3D and hooked up the same way.
