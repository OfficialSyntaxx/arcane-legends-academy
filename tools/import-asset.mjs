 ...
 *
 * Full pipeline: download → convert FBX/GLTF→GLB → cap texture size → print scale + texture status.
 *
 * NOTE: the texture cap was 512px, which is a 16x loss of texel density on a 2048px source and
 * was the main reason imported assets looked soft in-game. It is 2048 now — Draco (npm run
 * compress) is what keeps the deploy small, not texture downsizing.
 *
 * Usage:
 ...
 *   --resize <px>          Max texture size, 0 to skip (default: 2048)
...
const opts = { resize: 2048, target: 6.0, out: 'public/assets/models', name: null };
const positional = [];
